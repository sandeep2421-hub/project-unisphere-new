...
const HostelLeave = () => {
  ...
  const [selectedDate, setSelectedDate] = useState(leaveRequests[0].startDate);
          ...
          {dayLeaves.length > 0 && (
            <div className="day-events">
              {dayLeaves.slice(0, 2).map((leave, index) => (
                <div key={index} className="day-event-name" style={{ backgroundColor: leave.color }}>
                  {leave.type.split(' ')[0]}
                </div>
              ))}
              {dayLeaves.length > 2 && <div className="day-event-more">+{dayLeaves.length - 2}</div>}
            </div>
          )}
        </div>
      );
                ...
                {getLeavesByDate(new Date(selectedDate)).map((leave) => (
                  <div key={leave.id} className="date-event-card">
                    ...
                    <div className="date-event-content">
                      <p>{leave.category} {leave.reason} Status: {leave.status}</p>
                    </div>
              ...
              {filteredLeaves.length > 0 ? (
                <div className="events-list">
                  {filteredLeaves
                    .sort((a, b) => new Date(a.startDate) - new Date(b.startDate))
                    .map((leave) => (
                      <div key={leave.id} className={`event-card ${leave.startDate === selectedDate ? 'selected' : ''}`} onClick={() => setSelectedDate(leave.startDate)} style={{ cursor: 'pointer' }}>
                        <div
